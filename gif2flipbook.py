    ...
    try:
        for i in range(1, len(sys.argv)):
            if sys.argv[i][:7].strip().lower() == "border:":
                ...
                lines = False
            elif sys.argv[i].strip().lower() in [
                "no_size_increase",
                "no_size_increase",
            ]:
            ...
            # channel being on the right of the frames.
                ...
                three_dee_split = [
                    element
                    for element in sys.argv[i].strip().lower().split(":")
                    if element != ""
                ]
                if len(three_dee_split) > 1:
                    pixels_between_red_cyan = int(
                        re.findall(r"[\d]+", three_dee_split[1])[0]
                    )
...
if len(gif_files) > 0 and len(gif_files) <= 8:
    ...
    gif_names = [
        re.split(r"([.])", gif_names[i])[:-2][0] for i in range(len(gif_names))
    ]
    ...
    print(
        ...
        + str(len(gif_files))
        + " animation "
        + ("files") * (len(gif_files) > 1)
        + ("file") * (len(gif_files) == 1)
        + ":\n"
    )
    ...
    with alive_bar(len(gif_files)) as bar:
        ...
        for i in range(len(gif_files)):
            ...
            current_file_name_pixels_between_red_cyan = re.findall(
                r"[(](3d[ ]?[r|b]?[ ]?[\d]+[ ]?[r|b]?)[)]", gif_names[i].lower()
            )
            ...
            if current_file_name_pixels_between_red_cyan != []:
                ...
                if current_file_name_pixels_between_red_cyan[0] != "3d":
                    pixels_between_red_cyan_hits = re.findall(
                        r"[r|b]?[ ]?([\d]+)[ ]?[r|b]?",
                        current_file_name_pixels_between_red_cyan[0].replace("3d", ""),
                    )
                    if pixels_between_red_cyan_hits != []:
                        list_pixels_between_red_cyan.append(
                            int(pixels_between_red_cyan_hits[0])
                        )
                    else:
            ...
            current_file_brighten_percent = re.findall(
                r"[(]([-]?[\d]+)[ ]?%[)]", gif_names[i].lower()
            )
            if current_file_brighten_percent != []:
                list_brighten_percent.append(
                    int(current_file_brighten_percent[0]) / 100
                )
            else:
            ...
            if file_extension not in ["gif", "webp", "apng", "avif", "flif", "mng"]:
                try:
                    ...
                    if len(start_end) > 1:
                        ...
                        if "-" in start_end[0]:
                            start_time = [
                                element.strip() for element in start_end[0].split("-")
                            ]
                            if len(start_time) > 2:
                                start_time = (
                        ...
                        if "-" in start_end[1]:
                            end_time = [
                                element.strip() for element in start_end[1].split("-")
                            ]
                            if len(end_time) > 2:
                    ...
                    frame_timestamps = [
                        start_time + x * 1 / fps
                        for x in range((end_time - start_time + 1) * fps)
                    ]
                        ...
                        if frame_timestamps and frame_timestamps[0] <= j / video_fps:
                            cv2.imwrite(
                                os.path.join(
                                    ...
                                    "PNGS",
                                    gif_names[i],
                                    gif_names[i] + "-" + str(frame_counter) + ".png",
                                ),
                                frame,
                            )
                            frame_counter += 1
                    ...
                    sys.exit(
                        file_extension
                        + " files are not supported by this application."
                ...
                with Image.open(gif_files[i]) as gif_object:
                    try:
                        ...
                        sys.exit(
                            file_extension
                            + " files are not supported for flipbook generation."
    ...
    for i in range(len(gif_names)):
        ...
        if frame_durations != []:
            print(
                ...
                + gif_names[i]
                + ": "
                + str(gif_number_of_frames[i])
                + " frames, with a fps of "
                + str(animation_fps)
            )
        else:
    ...
    print(
        "\n\nCurrently generating a PDF document with a total of "
        + str(maximum_frame_number)
        + " frames.\n"
    )
    with alive_bar(maximum_frame_number) as bar:
        ...
        for i in range(maximum_frame_number):
            ...
            if len(gif_files) > 4:
                blank_canvas_reverse = Image.open(
                    os.path.join(cwd, "blank_canvas.jpg")
                ).convert("RGB")
            ...
            for j in range(len(gif_files)):
                if i == 0:
                    ...
                    if list_three_dee[j] == True:
                        ...
                        height_pixel_adjustment = math.floor(
                            list_pixels_between_red_cyan[j] * aspect_ratio
                        )
                        ...
                        anaglyph_frame_dimensions.append(
                            [
                                frame_width + list_pixels_between_red_cyan[j],
                                frame_height,
                                height_pixel_adjustment,
                            ]
                        )
                ...
                frame_k = Image.open(
                    os.path.join(gif_png_paths[j][png_index_list[j][i]])
                ).convert("RGB")
                ...
                # values.
                if resize_factor != 1:
                    frame_k = frame_k.resize(
                        gif_dimensions[j], resample=Image.Resampling.LANCZOS
                    )
                ...
                if list_three_dee[j] == True:
                    ...
                    anaglyph_frame = Image.new(
                        "RGB", anaglyph_frame_dimensions[j][:2], (255, 255, 255)
                    )
                    ...
                    if list_red_right[j] == False:
                        ...
                        # "x" axis.
                        anaglyph_frame.paste(
                            cyan_frame, (list_pixels_between_red_cyan[j], 0)
                        )
                        anaglyph_frame.paste(red_frame, (0, 0), red_frame)
                    else:
                        # The reverse is true if list_red_right[j] == True
                        anaglyph_frame.paste(
                            red_frame, (list_pixels_between_red_cyan[j], 0)
                        )
                    ...
                    # that animation.
                    frame_k = anaglyph_frame.crop(
                        (
                            list_pixels_between_red_cyan[j],
                            math.floor(0.5 * height_pixel_adjustment),
                            list_pixels_between_red_cyan[j]
                            + unextended_gif_dimensions[j][0],
                            math.floor(0.5 * height_pixel_adjustment)
                            + unextended_gif_dimensions[j][1],
                        )
                    )
            ...
            def text_image(number, numbers_font):
                ...
                page_number_text = Image.new(
                    "RGBA", page_number_size, (255, 255, 255, 0)
                )
            ...
            page_number_text, page_number_size = text_image(
                maximum_frame_number - i, numbers_font
            )
            ...
            if len(gif_files) > 4:
                ...
                page_number_half_width = page_number_size[0] / 2
                blank_canvas_reverse.paste(
                    page_number_text.rotate(180),
                    (
                        math.floor(2550 * 0.25 - page_number_size[0] / 2),
                        math.floor(3300 / 2 - border - page_number_size[1]),
                    ),
                )
                blank_canvas_reverse.paste(
                    page_number_text.rotate(180),
                    (
                        math.floor(2550 * 0.75 - page_number_size[0] / 2),
                        math.floor(3300 / 2 - border - page_number_size[1]),
                    ),
                )
                blank_canvas_reverse.paste(
            ...
            if len(gif_files) > 4:
                blank_canvas_reverse = blank_canvas_reverse.resize(
                    (
                        round(2550 * pdf_resolution / 300),
                        round(3300 * pdf_resolution / 300),
                    ),
    ...
    for i in range(len(gif_names)):
        ...
        if FrameNumber_PILImage[i][0][0] == 0 and FrameNumber_PILImage[i][1][0] == 1:
            FrameNumber_PILImage[i] = [
                FrameNumber_PILImage[i][j][1] for j in range(0, gif_number_of_frames[i])
            ]
        else:
            FrameNumber_PILImage[i] = [
                FrameNumber_PILImage[i][j][1]
                for j in range(gif_number_of_frames[i] - 1, -1, -1)
            ]
        print(
        ...
        FrameNumber_PILImage[i][0].save(
            os.path.join(final_pdf_path, gif_names[i] + ".gif"),
            save_all=True,
            append_images=FrameNumber_PILImage[i][1:],
            duration=gif_frame_duration_ms[i],
            loop=0,
        )

else:
    print(
        "Please include between one and eight GIF files inclusively within the GIFS folder."
    )
